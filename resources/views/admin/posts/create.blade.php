...
@section('page-title', 'Tambah Berita & Event')

@section('content')
<div class="bg-white rounded-lg shadow-md overflow-hidden">
    <div class="p-6 border-b">
        <h2 class="text-xl font-semibold text-gray-800">Form Tambah Berita & Event</h2>
    </div>
    ...
    <form action="{{ route('admin.posts.store') }}" method="POST" enctype="multipart/form-data" class="p-6">
        ...
        <div class="mb-4">
            ...
            <select name="kategori_id" id="kategori_id" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                ...
                <option value="1" {{ old('kategori_id') == 1 ? 'selected' : '' }}>Berita</option>
                <option value="2" {{ old('kategori_id') == 2 ? 'selected' : '' }}>Event</option>
                <option value="3" {{ old('kategori_id') == 3 ? 'selected' : '' }}>Jurusan</option>
            </select>
        </div>
        
        <!-- Field khusus Event -->
        <div id="event-fields" class="mb-4 hidden">
            <div class="mb-4">
                <label for="tanggal" class="block text-gray-700 text-sm font-bold mb-2">Tanggal Event</label>
                <input type="date" name="tanggal" id="tanggal" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('tanggal') }}">
            </div>
            <div class="mb-4">
                <label for="waktu_mulai" class="block text-gray-700 text-sm font-bold mb-2">Waktu Mulai (WIB)</label>
                <input type="time" name="waktu_mulai" id="waktu_mulai" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('waktu_mulai') }}">
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="tiket" id="tiket" placeholder="cth: Gratis (Wajib Registrasi)" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('tiket') }}">
            </div>
            <div class="mb-4">
                <label for="kapasitas" class="block text-gray-700 text-sm font-bold mb-2">Kapasitas Peserta</label>
                <input type="number" name="kapasitas" id="kapasitas" min="0" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('kapasitas') }}">
            </div>
        </div>
